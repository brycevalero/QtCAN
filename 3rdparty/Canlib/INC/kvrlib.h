...
typedef enum {
  ...
  kvrERR_XML_VALIDATION    = -14, /**< XML-file validation failed. */
  kvrERR_BUFFER_TOO_SMALL  = -15, /**< The buffer provided was not large enough to contain the requested data. */
...
/**
 * Verify that the xml buffer complies with both the DTD and internal restrictions. <br>
 * If the XML input creates any errors and err_buffer is to small to hold the resulting
 * error message, \ref kvrERR_BUFFER_TOO_SMALL will be returned and err_buffer will
 * hold a truncated error message. If the err_buffer is big enough,
 * \ref kvrERR_XML_VALIDATION will be returned.
 *
